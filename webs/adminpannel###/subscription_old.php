	...
	<body class="custom-tabel">
		...
		<div class="top-bg-img"></div>
			...
			<div id="page-content-wrapper">
				...
				<!-- Page content-->
				<div class="container-fluid subscription_plan_S content__up <?php if($rowSubs['plan_interval']=='Lifetime'){echo 'full_screen';} ?> ">
					...
					<h1 class="mt-4">Subscription 
						...
						<?php }else{ ?>
							<label class="bg-danger rounded  sub_cancel_S">Cancelled</label>
						<?php } ?>	
					</h1>
             ...
             <?php if($rowSubs['plan_interval']!="Lifetime"){ ?>            
					<div class="details  profile_tabs ">
						<h3>Subscription Details 
							<?php if($rowSubs['is_active'] == 1){ ?>
							<span data_cancle="https://websitespeedy.com/payment/cancle_subscription.php?sid=<?=base64_encode($rowSubs['id'])?>" class="text-danger subs_cancle btn btn-light">Cancel Subscription</span>
						<?php } ?>
							</h3>
...
<script>	$(".cancle_btn").click(function(){
   ...
   // alert(c+","+p);
			...
			Swal.fire({
				...
				<!-- End for add on -->
            <div class="profile_tabs inner__table__wrapper">
			<div class="log">
...
// $log1 = getTableData( $conn , " user_subscriptions_log " , " stripe_customer_id ='".$rowSubs['stripe_customer_id']."' ","order by id desc",1 ) ;
									...
									<tr>
										...
										<td class="subscriptions_status__td">
											<span class="<?php echo ucwords(str_replace("_"," ",$subs['subscriptions_status'])); ?>">
											<?php

												$c = $subs['subscriptions_status'];
												if($c=="canceled") {
													$c = "Cancelled";
												}
												echo ucwords(str_replace("_"," ",$c));
											?>
											</span>
										</td>
