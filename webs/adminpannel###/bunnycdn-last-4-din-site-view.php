...
for ($i=0; $i < 4 ; $i++) { 
    ...
    if ( $i == 0 ) {
        ...
        $log_date = date("m-d-y",strtotime("-$i day")) ;
    }

    echo "log_date : ".$log_date."<hr>" ;

    $curl_url = 'https://logging.bunnycdn.com/'.$log_date.'/1394947.log'; 
    echo "curl_url : ".$curl_url."<hr>" ;
    ...
    if ( count($unique_urls) > 0 ) {
        ...
        foreach ($unique_urls as $key => $url) {
            ...
            $query = $conn->query("SELECT * FROM `temp_site_visit_count` WHERE site_url LIKE '$url' ") ;
            if ( $query->num_rows > 0  ) {
                ...
                if ( $flag == 1 ) {
                    ...
                    if ( count($update_arr) > 0 && !empty($update_str) ) {

                        $log_date = $conn->real_escape_string($log_date) ;

                        if ( $i == 0 ) {
                            $update_str .= " , run1_date = '$log_date' " ; 
                        }
                        elseif ( $i == 1 ) {
                            $update_str .= " , run2_date = '$log_date' " ; 
                        }
                        elseif ( $i == 2 ) {
                            $update_str .= " , run3_date = '$log_date' " ; 
                        }
                        elseif ( $i == 3 ) {
                            $update_str .= " , run4_date = '$log_date' " ; 
                        }

                        // die($update_str) ;

                        $sql = " UPDATE `temp_site_visit_count` SET ".$update_str." WHERE id = $id ; " ;
                        echo "sql : ".$sql."<br>" ;
                        $conn->query($sql) ;
                    }
                ...
                $sql = " INSERT INTO `temp_site_visit_count`( `site_url`, `view_count`, `date` , `bandwidth` , `total_view_count` , `total_bandwidth` , `run1_date` ) VALUES ( '$url' , '$view_count' , '$log_date' , '$bandwidth' , '$view_count' , '$bandwidth' , '$log_date' ) ; " ;
