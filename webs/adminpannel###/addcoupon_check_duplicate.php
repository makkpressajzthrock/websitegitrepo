...
function SaveCoupon($conn, $CouponName, $coupon_code, $coupon_category, $coupon_for, $location,$duration, $coupon_id, $DiscountType, $DiscountAmount, $StartDate, $ExpiryDate, $DiscountPlan, $enable, $coupon){
...
if($DiscountType=="Lifetime"){
	...
	$DiscountAmount = 0;
			 ...
			 echo  $inst ="INSERT INTO `coupons`(`name`, `code`,`coupon_category`, `strip_coupon_id`, `form_location_value`, `location`, `duration`, `type`, `discount_amount`, `number_of_uses`, `uses_per_customer`, `start_date`, `expiry_date`, `plan_category_id`, `status`, `Strip_coupon_json`) 
					  		  ...
					  		  VALUES ('".mysqli_real_escape_string($conn, $CouponName)."','$coupon_code','$coupon_category','$coupon_id','$coupon_for','$location','$duration','$DiscountType','$DiscountAmount',0,1,'$StartDate', '$ExpiryDate','$DiscountPlan','$enable', '".mysqli_real_escape_string($conn, json_encode($coupon))."')";
    ...
    if($query->num_rows <= 0){ 	
			...
			if($DiscountType == "Percentage"){
				...
				if($coupon_for == "Other" || $coupon_for == "Both"){
					...
					if($coupon_id!="")
					{   $location = "Other";
					 	SaveCoupon($conn,$CouponName,$coupon_code, $coupon_category, $coupon_for,$location,$duration,$coupon_id,$DiscountType,$DiscountAmount,$StartDate,$ExpiryDate,$DiscountPlan,$enable,$coupon);
					}
				}
