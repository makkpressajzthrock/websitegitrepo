...
function update_pagespeed_record($conn,$website_id,$highest_reanalyze,$speedtype,$parent_website=0,$blank_record=NULL,$status="nonew") {
	...
	if ( $status == "nonew" ) {
		...
		if ( empty($parent_website) ) {
			$query2 = $conn->query(" SELECT * FROM temp_pagespeed_report WHERE `website_id` = '$website_id' AND plateform LIKE 'desktop' AND id = '$highest_reanalyze' ; ");
		}
		else {
			$query2 = $conn->query(" SELECT * FROM temp_pagespeed_report WHERE `website_id` = '$website_id' AND plateform LIKE 'desktop' AND id = '$highest_reanalyze' AND parent_website = '$parent_website' ; ");
		}
	...
	if ($query2->num_rows > 0) {
		...
		if ( empty($parent_website) ) {
			$query3 = $conn->query(" SELECT id FROM pagespeed_report WHERE website_id = '$website_id' AND parent_website = 0 AND (fetchTime = '' OR fetchTime IS NULL) ORDER BY `id` DESC LIMIT 1; ") ;
		}
		else {
			$query3 = $conn->query(" SELECT id FROM pagespeed_report WHERE website_id = '$website_id' AND parent_website = '$parent_website' AND (fetchTime = '' OR fetchTime IS NULL) ORDER BY `id` DESC LIMIT 1; ") ;
		}


		if ( $query3->num_rows > 0 ) {

			$data_pd = $query3->fetch_assoc() ;

			$pr_id = $data_pd["id"] ;

			$sql = " UPDATE `pagespeed_report` SET `fetchTime`='$fetchTime' , `environment`='$environment' , `runWarnings`='$runWarnings' , `configSettings`='$configSettings' , `audits`='$audits' , `categories`='$categories' , `categoryGroups`='$categoryGroups' , `i18n`='$i18n' , `speed_data`= 2 WHERE `id`='$pr_id' " ;
		
			if ( $conn->query($sql) === TRUE ) {

				remove_temp_speed_data($conn,$website_id,$parent_website,"desktop") ;
				...
				$ps_fcp = $audits_d["first-contentful-paint"]["numericValue"] ;
				$ps_lcp = $audits_d["largest-contentful-paint"]["numericValue"] ;
				$ps_cls = $audits_d["cumulative-layout-shift"]["numericValue"] ;
				$ps_tbt = $audits_d["total-blocking-time"]["numericValue"] ;
				$ps_si = $audits_d["speed-index"]["displayValue"] ;

				$stats = array( 'ps_desktop'=>$ps_desktop , 'ps_accessibility'=>$ps_accessibility , 'ps_best_practices'=>$ps_best_practices , 'ps_seo'=>$ps_seo , 'ps_fcp'=>$ps_fcp , 'ps_lcp'=>$ps_lcp , 'ps_cls'=>round($ps_cls, 3) , 'ps_tbt'=>$ps_tbt , 'ps_performance'=>$ps_performance , 'pr_id' => $pr_id , 'blank_record' => $blank_record , 'ps_si'=>$ps_si );
...
if ( isset($_POST["action"]) && ($_POST["action"] == "reanalyze-speed-compare-update") ) {
	...
	$return = array(
		'url1' => array( 'id' => $website_id , 'url' => $website_url , 'parent' => 0 , 'wi_status' => '' , 'desktop_highest_reanalyze' => '' , 'stats' => '' , 'desktop_less' => '' , 'platform_less' => '' ) ,
		'url2' => array( 'id' => $additional1_id , 'url' => $additional1_url , 'parent' => $website_id , 'wi_status' => '' , 'desktop_highest_reanalyze' => '' , 'stats' => '', 'desktop_less' => '' , 'platform_less' => '' ) ,
		'url3' => array( 'id' => $additional2_id , 'url' => $additional2_url , 'parent' => $website_id , 'wi_status' => '' , 'desktop_highest_reanalyze' => '' , 'stats' => '', 'desktop_less' => '' , 'platform_less' => '' ) ,
	...
	if ( $czs_flag == 1 ) {
		...
		$wi_highest_desktop_speed = 0 ;

		// desktop
		$query1 = $conn->query( " SELECT id , website_id , plateform , speed_score , reanalyze_count , parent_website FROM `temp_pagespeed_report` WHERE `website_id` = '$website_id' AND plateform LIKE 'desktop' AND parent_website = 0 ORDER BY reanalyze_count ASC; " );
		...
		if ( $wi_desktop_less == 1 ) {
			...
			remove_temp_speed_data($conn,$website_id,0,'desktop') ;
		}
		elseif ( $wi_desktop_highest_reanalyze == 0 ) {
			// nonew & no update
			$wi_status = "nonew" ;

			// create a new row with same score
			$stats = update_pagespeed_record($conn,$website_id,$wi_desktop_highest_reanalyze,$speedtype,0,NULL,$wi_status) ;
		}
		elseif ( $wi_desktop_highest_reanalyze > 0 ) {
			...
			if ( 0 <= $wi_desktop_speed_difference && $wi_desktop_speed_difference <= 5  ) {
		...
		if ( !empty($additional1_id) ) {
			...
			$aw1_highest_desktop_speed = 0 ;

			// mobile
			$query3 = $conn->query( " SELECT id , website_id , plateform , speed_score , reanalyze_count , parent_website FROM `temp_pagespeed_report` WHERE `website_id` = '$additional1_id' AND plateform LIKE 'desktop' AND parent_website = '$website_id' ORDER BY reanalyze_count ASC; " );
			...
			$stats = [] ;
			$platform_less = "" ;
			if ( $aw1_desktop_less == 1 ) {
				// popup 
				$aw1_status = "popup" ;
				$platform_less = "desktop" ;
				$popup_count++ ;
				remove_temp_speed_data($conn,$additional1_id,$website_id,"desktop") ;
			}
			elseif ( $aw1_desktop_highest_reanalyze == 0 ) {
				// nonew & no update
				$aw1_status = "nonew" ;

				// create a new row with same score
				$stats = update_pagespeed_record($conn,$additional1_id,$aw1_desktop_highest_reanalyze,$speedtype,$website_id,NULL,$aw1_status) ;
			}
			elseif ( $aw1_desktop_highest_reanalyze > 0 ) {
				// done & update both mobile , desktop record
				$aw1_status = "done" ;
				...
				if ( 0 <= $aw1_desktop_speed_difference && $aw1_desktop_speed_difference <= 5  ) {
					$manual_audit_needed = 1 ;
		...
		if ( !empty($additional2_id) ) {
			...
			if ( $query5->num_rows > 0 ) {
				...
				$aw2_desktop_less = 1 ;
				foreach ($temp_pagespeed_reports as $key => $tpr_data) {
					if ( (float)$tpr_data["speed_score"] > $additional2_wos_desktop ) {
			...
			$stats = [] ;
			$platform_less = "" ;
			if ( $aw2_desktop_less == 1 ) {
				// popup 
				$aw2_status = "popup" ;
				$platform_less = "desktop" ;
				$popup_count++ ;
				remove_temp_speed_data($conn,$additional2_id,$website_id,'desktop') ;
			}
			elseif ( $aw2_desktop_highest_reanalyze == 0 ) {
				// nonew & no update
				$aw2_status = "nonew" ;

				$stats = update_pagespeed_record($conn,$additional2_id,$aw2_desktop_highest_reanalyze,$speedtype,$website_id,NULL,$aw2_status) ;
			}
			elseif ( $aw2_desktop_highest_reanalyze > 0 || $aw2_desktop_highest_reanalyze > 0 ) {
				// done & update both mobile , desktop record
				$aw2_status = "done" ;
				...
				if ( 0 <= $aw2_desktop_speed_difference && $aw2_desktop_speed_difference <= 5  ) {
					$manual_audit_needed = 1 ;
