...
if ( count($unique_urls) > 0 ) {
    ...
    foreach ($unique_urls as $key => $url) {
        ...
        if ( $query->num_rows > 0  ) {
            ...
            $conn->query(" INSERT INTO `site_visit_count`( `site_url`, `view_count`, `date` , `bandwidth` , `total_view_count` , `total_bandwidth` ) VALUES ( '$url' , '$view_count' , '$log_date' , '$bandwidth' , '$view_count' , '$bandwidth' ) ; ") ;
