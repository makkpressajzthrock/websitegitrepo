...
<html>
   ...
   <body class="sdsds">
      <div class="container payment_s">
                ...
                if($result->num_rows > 0){ 
                  ...
                  //   print_r( $subscrData);die;
                     ...
                     if($plan_id =='29' || $plan_id=='30'){
                        ...
                        $paid_amount = number_format($subscrData['paid_amount'],2,".",","); 
                     ...
                     //  echo "<pre>";print_r($subscrData); die;
                    $stripe_subscription_id = $subscrData['stripe_subscription_id']; 
                    $paid_amount = number_format($subscrData['paid_amount'],2,".",",");  
                    ...
                    $customer_phone = $subscrData['payer_phone'];  //123
                    $customer_selfInstYes = $subscrData['self_install']; //123
                    if($plan_id=='4' || $plan_id=='15'){
                       $page_view = $subscrData['requested_views']; //123
                     }else{
                       $page_view = $subscrData['page_view']; //123
                    }
                    ...
                    require_once("../adminpannel/generate_script_paid.php");
                        ...
                        <?php
                           ...
                           $emailContent = $resultEmail->fetch_assoc(); 
                              ...
                              $emailVariables = array("plan" => $plan_name, "detail_link" => HOST_URL.'payment/payment-status.php?sid='.$_GET['sid']);
         ...
         <!-- //123 -->
         <?php include(__DIR__ . '/payment-statement-content.php'); ?>
         
         <table style="width: 100%;padding: 10px 40px;margin-bottom: 10px;background: #f2f2f2; font-family:arial;">
            <tr>
    ...
    border-radius: 5px;transition: all 250ms ease;font-weight: normal;min-width: 70px;text-align: center;" href="<?=HOST_HELP_URL?>faqs#pricing_billing_faq" target="_blank" >Help</a>
         ...
         <table style="max-width: 700px;margin: auto;width: 100%;padding:24px 20px;box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;margin: 30px auto; font-family:arial ;">
            <?php   sleep(1); if(!empty($subscr_id)){ ?>
            <thead> <tr><td colspan="2"><h3 style="color:#04AA6D;margin:0;font-size: 24px;text-align: center;" class="<?php echo $status; ?>"><?php echo $statusMsg; ?></td></tr></h3></thead>
            <tbody style="font-size:15px;">

            <tr><td style="padding-bottom:8px;" colspan="2"><h2 style="font-size:16px; margin:24px 0 0; font-family:arial ;">Payment Information</h2></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Reference Number: </td><td> <?php echo $subscr_id; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Subscription ID:  </td><td><?php echo $stripe_subscription_id; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Paid Amount:  </td><td>$<?php echo $paid_amount;?>&nbsp;USD</td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Status:  </td><td><?php echo $subscr_status; ?></td></tr>
            <tr><td style="padding-bottom:8px;" colspan="2"><h2 style="font-size:16px; margin:16px 0 0; font-family:arial ;">Subscription Information</h2></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Plan Name:  </td><td><?php echo $plan_name; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Amount: </td><td>$<?php echo $plan_amount.' '."USD"; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Plan Interval:  </td><td><?php echo $plan_interval; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Period Start:  </td><td><?php echo $plan_period_start; ?></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Period End:  </td><td><?php echo $plan_period_end; ?></td></tr>
            <tr><td style="padding-bottom:8px;" colspan="2"><h2 style="font-size:16px; margin:16px 0 0; font-family:arial ;">Customer Information</h2></td></tr>
            <tr><td style="padding:8px 8px 8px 15px; width:50%;">Email:  </td><td><?php echo $customer_email; ?></td></tr>
            <!-- //123 -->
            <!-- <p><b>Contact No:</b> <?php echo $customer_phone; ?></p> -->

            <tr><td style="padding:16px 0 0; text-align:center; " colspan="2">
            
            
            <?php 
             //selfInstallation//123
             if($customer_selfInstYes =='yes'){
                $selfInsBtn =   '<a style="margin-top:20p;xcolor: #fff; display: block; width: fit-content; margin:24px auto 0; background: #f23640; text-decoration: none;  padding: 5px 15px;  border-radius: 5px;  transition: all 250ms ease;"  href="'.HOST_URL.'adminpannel/project-dashboard.php?project='.base64_encode($website_id).'">Continue</a>';
                //  $selfInsBtn = '';
               }else{
                  $selfInsBtn = '<a  style="color: #fff; display: block; width: fit-content;  margin: 24px auto 0;  background: #f23640;   text-decoration: none;  padding: 5px 15px;  border-radius: 5px;   transition: all 250ms ease;" href="'.HOST_URL.'adminpannel/project-dashboard.php?project='.base64_encode($website_id).'">Continue</a>';
               }
               //123
               ?>
         ...
         <?php
            ...
            if($usercount == 1){
               ...
               ?>
                 <a style="color: #fff;  display: block; width: fit-content; margin: 24px auto 0; background: #f23640;  text-decoration: none;  padding: 5px 15px; border-radius: 5px; transition: all 250ms ease;" href="/adminpannel/my-subscriptions.php">Continue</a>
            <?php }
