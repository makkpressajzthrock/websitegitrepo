...
<!--             <div class="order_summary">
        ...
        <div class="payble_amt">Payble Amount: <?php echo  '$ <span class="payble_amt_val">'.$total_price.'</span>'; ?></div>
    ...
    <div class="panel-body">
        ...
        <input type="hidden" id="payer_email" class="form-control" name="payer_email"   value="<?php echo $sele_run2['email'];  ?>">
       ...
       <?php }
            ...
            <form id="subscrFrm">
                ...
                <input type="hidden" name="requested_views" id="requested_views" value="<?=$resultCart['requested_views'];?>" />
                <input type="hidden" name="requested_price" id="requested_price" value="<?=$resultCart['price'];?>" />
        ...
        if(coupon_code == ""){
            ...
            setTimeout(function(){$("#coupon_err").html('');},10000);
               ...
               $.ajax({
                    ...
                    method:"POST",
                    dataType:"JSON",
                    data:{subscr_plan:subscr_plan , coupon_code:coupon_code, 'location':'Other'}
                }).done(function(response)
                {
                    ...
                    if ( response.status == 1 ) {
                         ...
                         if(response.type=="Percentage" && response.discount != 100)
                         {
                            ...
                            final_p = (cart_Price - (cart_Price*response.discount/100)).toFixed(2);
                         }
                         else if(response.type=="Amount"){
                             final_p =  (cart_Price-response.discount+cart_tax_price);
                             final_p =  final_p.toFixed(2); 
...
$("body").on("click","#submitBtn_lifetime",function(){
    ...
    console.log(contact_number)
                                                                                                //123
    if(fname!="" && emailid!="" && country!="" && adr!="" && city!="" && zip!="" && contact_number!='')
    {
...
$('.getItNowForm').on('submit',function(e){
    ...
    $.ajax({
        ...
        success : function(response){
            ...
            if(obj.status == 1){
                // console.log(999);
                window.location.href = `./payment-status.php?sid=${obj.subs_id}&change_id=`;
