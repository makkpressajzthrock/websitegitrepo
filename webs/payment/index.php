...
//123
$sele44 = "SELECT country,firstname,lastname,email,phone,country_code FROM `admin_users` WHERE `id` = '".$managerId."'";
        ...
        <form id="biling-form" action="#" method="POST" >
           ...
           <input type="hidden" name="sid_id" id="sid_id" value="<?php echo $resultCart['website_id']; ?>" />
            ...
            <div class="sm__line__flex" >
                <div class="form-group">
                    ...
                    <select class="form-control"  data-id-country="<?=$SelectedCountry?>" id="country" data-country="country" class="form-control" name="country" required>
                        <option  name="option" value="">Select Country</option>
                        <?php
                            ...
                            foreach ($list_countries as $key => $country_data) {
                                ...
                                ?>
                                <option <?php if($SelectedCountry==$country_data["id"]){ echo "selected"; } ?>  value="<?=$country_data["id"]?>" <?=$selected?> data-sortname="<?=$country_data["sortname"]?>" ><?=$country_data["name"]?></option>
                                <?php
                            }
                        ?>
                    </select>
                ...
                <input type="text" id="adr" class="form-control" value="<?php echo $sele_run2['address'];  ?>"   name="address" placeholder="Building & Stress address" required>
                </div>
            ...
            <div class="sm__line__flex" >
              ...
              <!--//123  -->
                <div class="form-group">
                    <label for="country_code">Country Code <span class="required-star">*</span></label>
                        <select id="country_code" class="form-control" name="country_code" required>
                            ...
                            ?>          
                                    <option value='+<?=$country_data["phonecode"]?>'  <?php if($country_data["id"] == $SelectedCountry){echo "selected";} ?>  ><?=$country_data["name"]?> +<?=$country_data["phonecode"]?></option>
                            <?php
                                    }
                            ?>
                        </select>
            ...
            <div class="sm__line__flex" >
                <div class="form-group">
                    <label for="contact_number">Phone Number <span class="required-star">*</span></label>
                    <input type="text" maxlength="20" minlength="7" id="contact_number" class="form-control" name="contact_number" value="<?php echo $sele_run44['phone'];?>"   required>
                </div> 
...
$("#validate_btn").on('click', function(){
                  ...
                  //123
                  if(!isvalid("#contact_number")){
                  markinviled("#contact_number");
                  flag = 1;
                  }
                  else{
...
<script>
    $(document).ready(function() {
      $('#contact_number').on('keypress', function(event) {
        ...
        // Allow numbers (48-57), plus sign (43), and backspace (8)
