...
async function handleSubscrSubmit(e) {
    ...
    phone = phone.trim() ;
                                                                                        //123
    if(fname!="" && emailid!="" && country!="" && adr!="" && city!="" && zip!="" && phone!="" )
    {    
    ...
    fetch("payment_init_ind.php", {
        ...
        body: JSON.stringify({ request_type:'create_customer_subscription', subscr_plan_id: subscr_plan_id, name: customer_name, email: customer_email, phone : customer_phone, change_id: change_id,t_Price:t_Price,sid_id:sid_id,coupon_id:coupon_id,gst_number:gst_number,discount_amount:discount_amount,with_trial:with_trial,plan_name:plan_name,plan_type:plan_type }),
    })
...
if ( document.querySelector("#subscrTrialFrm") ) {
    ...
    async function handleSubscrTrialSubmit(e) {
        ...
        let fname = document.getElementById("fname").value;
        let emailid = document.getElementById("emailId").value;
        let phoneNo = document.getElementById("contact_number").value; //123
        let country = document.getElementById("country").value;
        let adr = document.getElementById("adr").value;
        let city = document.getElementById("city").value;
        let zip = document.getElementById("zip").value;
