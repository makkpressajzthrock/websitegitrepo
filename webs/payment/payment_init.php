...
if ($jsonObj->request_type == 'create_customer_subscription') {
    ...
    $requested_views = !empty($jsonObj->requested_views) ? $jsonObj->requested_views : ''; //123
    $requested_price = !empty($jsonObj->requested_price) ? $jsonObj->requested_price : ''; //123
    ...
    if ( ($subscr_plan_id == '15' || $subscr_plan_id == '4') && !empty($requested_price) ) {
        $planPriceCents = round($requested_price * 100);
    }
    else {
    ...
    if (empty($api_error) && $customer) {
        try {
            ...
            $price = \Stripe\Price::create([
                ...
                /*** Start metadata ***/ 
                // 'metadata' => [ 'requested_views' => $requested_views ],
                'metadata' => [ 
...
elseif ($jsonObj->request_type == 'payment_insert') {
    ...
    if (!empty($payment_intent) && $payment_intent->status == 'succeeded') {
        ...
        if (!empty($prevRow)) {
            ...
            //123
            if($subscr_plan_id=='4'||$subscr_plan_id=='15'){
                
                $t_Price= $requested_price.".00";
            ...
            //123
               $sqlQ = "INSERT INTO user_subscriptions (user_id,plan_id,stripe_subscription_id,stripe_customer_id,stripe_payment_intent_id,paid_amount,total_tax,discount_amount,discount_code_id,plan_price,paid_amount_currency,plan_interval,payer_email,payer_phone,created,plan_period_start,plan_period_end,status,subscription_items_id,products_id,json_data,site_count,vat , requested_views , price ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
               $stmt = $db->prepare($sqlQ);
            //123
            $stmt->bind_param("iisssdsssssssisssssssssss", $db_user_id, $db_plan_id, $db_stripe_subscription_id, $db_stripe_customer_id, $db_stripe_payment_intent_id, $db_paid_amount, $db_paid_total_tax, $bd_discount_amount,$db_discount_code_id, $db_paid_total_price, $db_paid_amount_currency, $db_plan_interval, $db_payer_email, $db_payer_phone, $db_created, $db_plan_period_start, $db_plan_period_end, $db_status, $subscription_items_id, $products_id, $json_data, $site_count, $vat_number ,$requested_views,$requested_price);
            $db_user_id = $userID;
            ...
            // $db_requested_views = $requested_views ;
            // $db_requested_price = $requested_price ;
    ...
    if (empty($api_error) && $customer) {
        ...
        if (empty($api_error) && $session) {
            ...
            if ( $ustd_qcheck->num_rows > 0 ) {
                ...
                //123
                $db->query( " UPDATE `user_subscription_trial_data` SET `user_id`='$userID' ,`subscr_plan_id`='$subscr_plan_id',`name`='$name',`email`='$email', `phone`='$phone',`coupon_id`='$coupon_id',`vat_number`='$vat_number' ,`with_trial`='$with_trial',`country_label`='$country_label',`cancel_url`='$cancel_url',`planName`='$planName',`planPrice`='$planPrice',`planInterval`='$planInterval',`price`='$price',`count`='$count',`planPriceCents`='$planPriceCents',`change_id`='$change_id',`t_Price`='$t_Price',`sid_id`='$sid_id',`discount_amount`='$discount_amount',`tax_price`='$tax_price',`used`=0 WHERE `id`='$ustd_id' " ); 

            }
            else {                  
                //123
                $db->query( " INSERT INTO `user_subscription_trial_data`( `user_id`, `subscr_plan_id`, `name`, `email`, `phone`, `coupon_id`, `vat_number` , `with_trial`, `country_label`, `cancel_url`, `planName`, `planPrice`, `planInterval`, `price`, `count`, `planPriceCents`, `change_id`, `t_Price`, `sid_id`, `discount_amount`, `tax_price` , `stripe_customer_id` , `used` ) VALUES ( '$userID' , '$subscr_plan_id' , '$name' , '$email' , '$phone', '$coupon_id' , '$vat_number' , '$with_trial' , '$country_label' , '$cancel_url' , '$planName' , '$planPrice' , '$planInterval' , '$price' , '$count' , '$planPriceCents' , '$change_id' , '$t_Price' , '$sid_id' , '$discount_amount' , '$tax_price' , '$stripe_customer_id' , 0 ) " ); 
            }
