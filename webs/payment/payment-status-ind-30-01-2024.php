...
if(!empty($_GET['sid'])){ 
    ...
    $sqlQ = "SELECT S.*, P.name as plan_name, P.main_p as plan_amount, U.firstname, U.lastname, U.email,S.plan_id as plan_id, S.send_email FROM user_subscriptions as S LEFT JOIN admin_users as U On U.id = S.user_id LEFT JOIN plans as P On P.id = S.plan_id WHERE S.id = ?"; 
...
<?php
                ...
                $emailContent = $resultEmail->fetch_assoc(); 
                    ...
                    $emailVariables = array("plan" => $plan_name, "detail_link" => 'https://websitespeedy.com/payment/payment-status-ind.php?sid='.$_GET['sid']);
