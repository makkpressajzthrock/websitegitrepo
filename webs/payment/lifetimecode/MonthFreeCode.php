  ...
  $userID = $_SESSION['user_id'];
	...
	{	
		...
		$currency =  $_POST['currency'];
		 ...
		 $fetchCode = "SELECT * FROM coupons WHERE id = '$coupon_id' AND code ='$coupon_code' AND type ='Percentage' ";
		...
		if ($num_rows > 0 ) 
		{
            ...
            // End old subscription cancelled
				 ...
				 $plan_period_end =Date('Y-m-d', strtotime('+'.$subscription_duration.' month'));
                ...
                if($rowData['status']==2 && $rowData['uses_per_customer']!= 'unlimited'){  
				 ...
				 $queryL = "INSERT INTO user_subscriptions (user_id,plan_id,payment_method,stripe_subscription_id,stripe_customer_id,stripe_payment_intent_id,paid_amount, total_tax,plan_price,paid_amount_currency,site_count,plan_interval,plan_interval_count,payer_email,created,plan_period_start,plan_period_end,status,discount_code_id,requested_views) VALUES ('$userID','$subscr_plan_id','stripe','xxxxxxxxxxxx','xxxxxxxxxxx','xxxxxxxxxxxxx','0','0','0','$currency','1','Month Free','1','$customer_email',now(),now(),'$plan_period_end','succeeded','$coupon_code','$requested_views')"; 	
