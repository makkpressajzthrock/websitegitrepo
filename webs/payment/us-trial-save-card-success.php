    ...
    if (empty($api_error) && $session) {
	    ...
	    if (empty($api_error) && $setup_intent_data) {
	        ...
	        if ( $ustd_qcheck->num_rows > 0 ) {
				...
				$vat_number = $ustd_dcheck["vat_number"] ;
				...
				$vat_number = $_SESSION["customer_trial_data"]["vat_number"] ;
				...
				***/ 
		        ...
		        if (empty($api_error) && $price) {
		            ...
		            if ( $with_trial == 1 ) {
		                ...
		                // $seven_days_after = strtotime($today_date . " +3 hour");
		                // $seven_days_after = strtotime($today_date . " +10 minutes");
		            ...
		            if (empty($api_error) && $subscription) {
						...
						header("location: ".HOST_URL."payment/payment-status.php?sid=".base64_encode($payment_id)."&change_id=".$change_id) ;
						die() ;	 
