  ...
  content: "✖";
	...
	<body class="custom-tabel">
			...
			<div id="page-content-wrapper">
				...
				<div class="container-fluid billing_dashS content__up">
					...
					<ul class="menu profile_tabs">
						<li><a href="<?=HOST_URL?>adminpannel/manager_settings.php?active=profile" id="profile"><button type="button" data-select="Profile" class=" nav_btn nav_btn1 "><i class="las la-user"></i><Span>Profile</Span></button></a></li>
						<?php
						...
						?>
							...
							<li><a href="<?=HOST_URL."adminpannel/my-subscriptions.php" ?>" id="my_plan"><button type="button" data-select="my plan" class=" nav_btn nav_btn7"><svg height="24" id="myPlanSVG" fill="currentColor" version="1.1" viewBox="0 0 24 24" width="24"	 xmlns="http://www.w3.org/2000/svg" xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:svg="http://www.w3.org/2000/svg">
										...
										</svg><span>My Subscriptions </span></button></a></li>
		...
		<?php
			...
			$userSubscription = "SELECT user_subscriptions.id as userSubscriptionId,user_subscriptions.*, admin_users.id as adminUserId,admin_users.* FROM user_subscriptions INNER JOIN admin_users ON user_subscriptions.user_id = admin_users.id AND admin_users.id = '$user_id' and user_subscriptions.plan_interval <> 'Lifetime' and user_subscriptions.plan_interval <> 'Month Free'  order by  user_subscriptions.id desc";
...
if($user_data->num_rows >0){
			...
			while ($userData=mysqli_fetch_assoc($user_data))
			{
				...
				// echo $userData['userSubscriptionId']."<>";
		...
		?>
							<tr class="list__item">
								...
								<td><?php echo $userData['paid_amount']." ".strtoupper($userData['paid_amount_currency']); ?></td>
								<td><?php         $timedy= $userData['plan_period_start'];
                                           $vartime = strtotime($timedy);

                                             $datetimecon= date("F d, Y H:i", $vartime); echo $datetimecon ; ?></td>
								<td><?php   $timedy2= $userData['plan_period_end'];
								...
								<td>
									...
									<a download href="<?=HOST_URL?>adminpannel/generate-pdf.php?id=<?php echo $userSubscriptionId;?>" class="download_pdf btn btn-primary" target="blank"><i class="lar la-file-alt"></i></a>
