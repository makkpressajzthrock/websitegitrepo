				...
				<div class="container-fluid content__up dashboard_SS" >
					...
					$speed = getTableData( $conn , " pagespeed_report " , " website_id ='$project_id' " );
						...
						foreach ($projects as $key => $project_data) 
						{
							...
							// pagespeed_report -----------------
							$wd_query = $conn->query(" SELECT categories, mobile_categories FROM `pagespeed_report` WHERE `website_id` = '".$project_data["id"]."' AND `parent_website` = '0' ORDER BY `pagespeed_report`.`id` DESC LIMIT 1 ") ;
							...
							$plan_info_queryA = "SELECT * from plans WHERE id = '$plan__id'";

							if(!empty($project_data['id'])){
								$count_url=1;
							}

				$plan_info_queryB = $conn->query($plan_info_queryA);

				 $plan_info_query = $plan_info_queryB->fetch_assoc();
