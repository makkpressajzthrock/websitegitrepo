...
$id = base64_decode($_GET['id']);
 ...
 $userSubscription = "SELECT user_subscriptions_log.id as userSubscriptionId,user_subscriptions_log.*, admin_users.id as adminUserId,admin_users.*, admin_users.country_code as country_code FROM user_subscriptions_log INNER JOIN admin_users ON user_subscriptions_log.user_id = admin_users.id WHERE user_subscriptions_log.id = '$id' AND admin_users.id = '$user_id' ";
...
$stripe_subscription_id = $userData['stripe_subscription_id'];
 ...
 $userSubscriptions = "SELECT * FROM user_subscriptions WHERE stripe_subscription_id = '$stripe_subscription_id' AND user_id = '$user_id' ";
$user_datas = mysqli_query($conn,$userSubscriptions);
$userDatas=mysqli_fetch_assoc($user_datas);
...
$gstnumber = "";
if($userDatas['gst']!=""){
	$gstnumber = "GST Number: ".$userDatas['gst'];
}

$vatnumber = "";
if($userDatas['vat']!=""){
	$vatnumber = "VAT Number: ".$userDatas['vat'];
}
...
$header_gst_show = '' ;
if ( strtolower($userDatas["paid_amount_currency"]) != "usd" ) {
...
// echo " SELECT * FROM `boost_website` WHERE `subscription_id` = '".$userData['userSubscriptionId']."' " ;
 $bw_query = $conn->query(" SELECT * FROM `boost_website` WHERE `subscription_id` = '".$userDatas['id']."' and plan_type = 'Subscription' ") ;
...
$currency  = strtoupper($userDatas["paid_amount_currency"]); 
// $currency ="";
$summ = '
		<table style="margin-left:5px;width:100%;">
			<tr style="width:100%;">
				<td>Plan Price</td>
				<td style="text-align:end;padding-left:20px">'.number_format($userDatas['plan_price'],2).' '.$currency.'</td>
			</tr>';

if($userDatas['discount_amount']!="0"){

 $discount = $conn->query(" SELECT code FROM `coupons` WHERE `strip_coupon_id` = '".$userDatas['discount_code_id']."' ") ;
 $bw_data_data_d = $discount->fetch_assoc() ;

 $dc = $bw_data_data_d['code']?$bw_data_data_d['code']:'Code';
$summ .= '
		 
			<tr style="width:100%;">
				<td>Discount<br> ('.$dc.')</td>
				<td style="text-align:end;padding-left:20px">-'.number_format($userDatas['discount_amount'],2).' '.$currency.'</td>
			</tr>';

}


if($userDatas['total_tax']!=""){
...
$summ .= '
			...
			<tr style="width:100%;">
				...
				<td style="text-align:end;padding-left:20px">'.number_format($userDatas['total_tax'],2).' '.$currency.'</td>
			</tr>';
...
';
	...
	<div style="height:50px;  font-size:14px; text-align:right; border-bottom:1px solid lightgray; ">
		...
		<div style="font-size:24px; width:35%; height:40px; border-left:1px solid lightgray; float:right; padding-top:10px;" >'.$userDatas["paid_amount"].'    <span style="font-size:14px; text-transform:uppercase;">'.$userDatas["paid_amount_currency"].'</span>
