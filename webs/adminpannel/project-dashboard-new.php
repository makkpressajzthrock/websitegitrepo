            ...
            <div class="container-fluid content__up project__dashboard">
                ...
                <div class="profile_tabs dashboard_sn">
                    ...
                    <div class="card_con_ss">
                        ...
                        $wd_query = $conn->query("SELECT id , website_id , parent_website , desktop_score , mobile_score , updated_at , no_speedy FROM `pagespeed_report` WHERE `website_id` = '".$project_id."' AND `parent_website` = '0' AND no_speedy = 1 ORDER BY `pagespeed_report`.`id` asc LIMIT 1");
                        ...
                        if ($wd_query->num_rows > 0) {
                            ...
                            $wd_performance = $wd_data["mobile_score"];
                            $wd_mobile = $wd_performance."/100";
                            ...
                            /*** 
                            $wd_query = $conn->query("SELECT id , website_id , parent_website , desktop_score , mobile_score , updated_at , no_speedy FROM `pagespeed_report` WHERE `website_id` = '".$project_id."' AND `parent_website` = '0' ORDER BY `pagespeed_report`.`id` ASC LIMIT 1");
                        ...
                        <div class="row dash_overview" style="flex-wrap: wrap;">
                            ...
                            for ($i=2; $i < 4 ; $i++) { 
                                ...
                                $awu2_query = $conn->query(" SELECT id , url_priority , website_url , website_name FROM `additional_websites` WHERE `manager_id` = '$user_id' AND `website_id` = '$project_id' AND url_priority = $i ORDER BY id DESC LIMIT 1 ; ");

                                if ( $awu2_query->num_rows > 0 ) {

                                    $additional_url = $awu2_query->fetch_assoc() ;

                                    $additional_arr[$additional_url["url_priority"]] = array( "id" => $additional_url["id"] , "updated_at" => $additional_url["updated_at"] , "website_url" => $additional_url["website_url"] , "website_name" => $additional_url["website_name"] )  ;
                                    
                                    $ps_desktop = $ps_mobile = "complete Installation";

                                    $ps_query = $conn->query("SELECT id , website_id , parent_website , desktop_score , mobile_score , updated_at , no_speedy FROM pagespeed_report WHERE website_id = '".$additional_url["id"]."' AND parent_website = '$project_id'  AND no_speedy='1' ORDER BY id DESC LIMIT 1") ;
                            ...
                            $wd_query = $conn->query(" SELECT id , website_id , parent_website , desktop_score , mobile_score , updated_at , no_speedy , ws_status , blank_record FROM `pagespeed_report` WHERE `website_id` = '".$project_id."' AND `parent_website` = '0'  AND no_speedy = '0' AND initial_url='0' ORDER BY `pagespeed_report`.`id` DESC LIMIT 1") ;
                            ...
                            for ($i=2; $i < 4 ; $i++) { 
                                ...
                                if ( array_key_exists($i, $additional_arr) && is_array($additional_arr[$i]) ) {

                                    $additional_url = $additional_arr[$i] ;
                                    ...
                                    $ps_query = $conn->query("SELECT id , desktop_score , mobile_score , updated_at , no_speedy , parent_website , website_id , ws_status , speed_data , blank_record FROM pagespeed_report WHERE website_id = '".$additional_url["id"]."' AND parent_website = '$project_id' AND no_speedy = '0' ORDER BY id DESC LIMIT 1 ") ;
