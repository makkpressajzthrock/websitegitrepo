	...
	***/
	...
	if( $view_percentage>=100 ){
		...
		***/
		...
		if( $view_percentage >= 80 && $view_percentage < 100 ) {
...
if($sendemail == 1){
	...
	// set email log
	$conn->query("INSERT INTO email_logs (user_id, email_subject, email_message, created_at) VALUES('$user_id', '".$conn->real_escape_string($emailContent)."', '".$conn->real_escape_string($emailContent)."', CURDATE())");
}
