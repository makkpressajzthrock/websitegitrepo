				...
				<div class="profile_tabs">
					<div class="row">
						...
						<div class="col-md-12">
							...
							<form method="GET" class="w-50 float-right">
								...
								<?php
									...
									foreach ($months as $month_key => $month) {
										...
										$selected = ($get_month == $month_key) ? "selected" : "" ;

										?><option value="<?=$month_key?>" <?=$selected?> ><?=$month?></option><?php
									}
								?>
								</select>
								...
								<?php
									...
									for ($i = 0; $i < 4; $i++) {
										...
										$selected = ($get_year == $year) ? "selected" : "" ;
						...
						if ( !empty($_GET["month"]) && !empty($_GET["year"]) ) {
							...
							$qry_month = " select sum(paid_amount) as paid_amount  from user_subscriptions where paid_amount_currency = 'usd' and CONCAT(YEAR(created),MONTH(created)) = CONCAT($get_year,$get_month)  ";
						}

						// echo "qry_month : ".$qry_month ;
						...
						if ( !empty($_GET["month"]) && !empty($_GET["year"]) ) {
							...
							$qry_yearly = " select sum(paid_amount) as paid_amount  from user_subscriptions where paid_amount_currency = 'usd' and  CONCAT(YEAR(created)) = CONCAT($get_year) ";
						}

						// echo "qry_yearly : ".$qry_yearly ;
...
<path d="M620 500 c-13 -8 -12 -16 10 -55 l25 -46 -22 -39 c-27 -46 -28 -56 -8 -63 9 -4 24 11 45 48 l31 55 -31 55 c-17 30 -32 55 -33 55 -1 0 -9 -5 -17 -10z"/>
					...
					<!-- Start Total This Monthly Signup -->
					<?php
					   	// Get the current month and year

						$total_signups = 0 ;

						$sql = "SELECT COUNT(DISTINCT(boost_website.manager_id)) AS total_signups FROM boost_website JOIN admin_users ON boost_website.manager_id = admin_users.id WHERE admin_users.firstname NOT LIKE '%makkpress%' AND admin_users.lastname NOT LIKE '%makkpress%' AND admin_users.active_status = 1 AND MONTH(boost_website.created_at) = $get_month AND YEAR(boost_website.created_at) = $get_year";

						$ts_query = $conn->query($sql) ;

						if ( $ts_query->num_rows > 0 ) {
							$ts_data = $ts_query->fetch_assoc() ;
							$total_signups = $ts_data["total_signups"] ;
						}
					?>
					<div class="col-md-4">
						<div class="card">
							<div class="card-body">
								<div class="row">
									<h5>Total This Monthly Signup<span></span>
									</h5>
									<h4><?php echo $total_signups; ?></h4>
								</div>
								<div class="dash_icon">
									<svg xmlns="http://www.w3.org/2000/svg" version="1.0" width="100.000000pt" height="100.000000pt" viewBox="0 0 100.000000 100.000000" preserveAspectRatio="xMidYMid meet">
										<g transform="translate(0.000000,100.000000) scale(0.100000,-0.100000)" fill="currentColor" stroke="none">
					...
					<!-- End Total This Monthly Signup -->
						

					<!-- Start Total This Monthly Basic Plan -->
					<?php
						// Total This Monthly Basic Plan
						$total_basic_plan = 0 ;
						$sql = " SELECT count('user_subscriptions.id') as total_basic_plan FROM user_subscriptions JOIN admin_users ON user_subscriptions.user_id = admin_users.id WHERE admin_users.firstname NOT LIKE '%makkpress%' AND admin_users.lastname NOT LIKE '%makkpress%' AND admin_users.active_status = 1 AND user_subscriptions.is_active = 1 AND user_subscriptions.plan_id IN (29, 30) AND MONTH(user_subscriptions.created) = $get_month AND YEAR(user_subscriptions.created) = $get_year; ";

						// echo $sql ;
						
						$tbp_query = $conn->query($sql) ;

						if ( $tbp_query->num_rows > 0 ) {
							$tbp_total = $tbp_query->fetch_assoc() ;
							$total_basic_plan = $tbp_total['total_basic_plan'] ;
						}
					?>
					<div class="col-md-4">
						<div class="card">
							<div class="card-body">
								<div class="row">
									<h5>Total This Monthly Basic Plan<span></span></h5>
									<h4><?=$total_basic_plan;?></h4>
								</div>
								<div class="dash_icon">
									<svg xmlns="http://www.w3.org/2000/svg" version="1.0" width="100.000000pt" height="100.000000pt" viewBox="0 0 100.000000 100.000000" preserveAspectRatio="xMidYMid meet">
										<g transform="translate(0.000000,100.000000) scale(0.100000,-0.100000)" fill="currentColor" stroke="none">
					...
					<!-- END Total This Monthly Basic Plan -->


					<!-- Start Total This Monthly Paid Plan -->
					<?php
					   	// Get the current month and year
						$total_paid_plan = 0 ;

						// $sql = "SELECT  count('user_subscriptions.id') as total_paid_plan FROM user_subscriptions  JOIN admin_users ON user_subscriptions.user_id = admin_users.id  WHERE admin_users.firstname NOT LIKE '%makkpress%'  AND admin_users.lastname NOT LIKE '%makkpress%'  AND admin_users.active_status = 1 AND user_subscriptions.is_active = 1  AND user_subscriptions.plan_id IN (2,4,14,15,26,27) AND MONTH(user_subscriptions.created) = $get_month AND YEAR(user_subscriptions.created) = $get_year;";

						$sql = "SELECT  count('user_subscriptions.id') as total_paid_plan FROM user_subscriptions  JOIN admin_users ON user_subscriptions.user_id = admin_users.id  WHERE admin_users.firstname NOT LIKE '%makkpress%'  AND admin_users.lastname NOT LIKE '%makkpress%'  AND admin_users.active_status = 1 AND user_subscriptions.is_active = 1  AND user_subscriptions.plan_id NOT IN (29,30) AND MONTH(user_subscriptions.created) = $get_month AND YEAR(user_subscriptions.created) = $get_year;";

						// echo $sql ;

						$tpp_query = $conn->query($sql) ;

						if ( $tpp_query->num_rows > 0 ) {
							$tbp_total = $tpp_query->fetch_assoc() ;
							$total_paid_plan = $tbp_total['total_paid_plan'] ;
						}
					?>
					<div class="col-md-4">
						<div class="card">
							<div class="card-body">
								<div class="row">
									<h5>Total This Monthly Paid Plan<span></span>
									</h5>
									<h4><?=$total_paid_plan;?></h4>
								</div>
								<div class="dash_icon">
									<svg xmlns="http://www.w3.org/2000/svg" version="1.0" width="100.000000pt" height="100.000000pt" viewBox="0 0 100.000000 100.000000" preserveAspectRatio="xMidYMid meet">
										<g transform="translate(0.000000,100.000000) scale(0.100000,-0.100000)" fill="currentColor" stroke="none">
					...
					<!-- END Total This Monthly Paid Plan -->
...
<hr>
						...
						<div class="col-md-4">
							...
							<?php
								...
								if ( !empty($_GET["month"]) && !empty($_GET["year"]) ) {
									...
									$qry_month = "SELECT SUM(paid_amount) AS paid_amount 
									FROM user_subscriptions 
									WHERE CONCAT(YEAR(created), MONTH(created)) = CONCAT($get_year, $get_month)
									AND paid_amount_currency = 'inr'
									AND stripe_subscription_id <> 'xxxxxxxxxxxx'";
								}
						...
						if ( !empty($_GET["month"]) && !empty($_GET["year"]) ) {
							...
							$qry_yearly_indian = " select sum(paid_amount) as paid_amount  from user_subscriptions where CONCAT(YEAR(created)) = CONCAT($get_year) AND paid_amount_currency ='inr' AND stripe_subscription_id <> 'xxxxxxxxxxxx'";
						}
