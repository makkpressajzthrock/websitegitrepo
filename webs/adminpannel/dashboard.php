...
<body class="custom-tabel">
            ...
            <div class="row custom-row-gap">
               <?php
                  ...
                  foreach ($projects as $key => $project_data) 
                  {
                  	...
                  	if ( $project_data['subscription_id'] == "111111" || $project_data['subscription_id'] == 111111 ) {
                  		...
                  		if( $tmp_user_data["user_type"] == "Dealify"  || $tmp_user_data["user_type"] == "DealFuel" ) {
                  			$newappsumo = 1 ;
                  		}
                  
                  		// print_r($tmp_user_data["user_type"]) ;
                  		if( $tmp_user_data["user_type"] == "AppSumo" && $tmp_user_data["sumo_new"] == "1" ) {
                  			$newappsumo = 1 ;
                  		}
               ...
               <div  style="display: none;" id="page-speed-table" data-project="<?=$project_data['id']?>" data-type="page-speed">
                  <?php
                     ...
                     ?>
                  <button type="button" class="btn btn-primary reanalyze-btn-new" data-website_name="<?=$project_data["website_url"]?>" data-website_url="<?=$project_data["website_url"]?>" data-website_id="<?=$project_data["id"]?>" data-additional="0">
                     ...
                     <!-- <i class="fa fa-refresh" aria-hidden="true"></i> Font Awesome fontawesome.com -->
                  </button>
               ...
               <?php
                  ...
                  $wd_query = $conn->query(" SELECT * FROM $pagespeed_report WHERE `website_id` = '".$project_data["id"]."' AND `parent_website` = '0' ORDER BY id DESC LIMIT 1 ") ;
               ...
               <div class="col-md-4 with__shadow">
                  <div href="javascript:void(0)" style="    text-decoration: none;">
                     <ul class="list-group">
                        ...
                        <li class="list-group-item"> Pages View <span class="float-right">
                           <!-- start page views update -->
                           <?php
                              $view_count = $total_view_label = 0 ;
                              
                              $sb_pid = $project_data["id"] ;
                              
                              $sb_pq = $conn->query(" SELECT website_url , plan_id , id , plan_type , subscription_id FROM `boost_website` WHERE `id` = '$sb_pid' LIMIT 1 ; ") ;
                              
                              if ( $sb_pq->num_rows > 0 ) {
                              
                                 $sb_pd = $sb_pq->fetch_assoc() ;

                                 if ( $sb_pd["plan_type"] == "Subscription" ) {
                                     $p_query = $conn->query(" SELECT user_subscriptions.id , user_subscriptions.requested_views , plans.page_view FROM user_subscriptions , plans WHERE user_subscriptions.id = '".$sb_pd["subscription_id"]."' AND plans.id = '".$sb_pd["plan_id"]."' ; ");
                                 }
                                 else {
                                     $p_query = $conn->query(" SELECT user_subscriptions_free.id , user_subscriptions_free.requested_views , plans.page_view FROM user_subscriptions_free , plans WHERE user_subscriptions_free.id = '".$sb_pd["subscription_id"]."' AND plans.id = '".$sb_pd["plan_id"]."' ; ");
                                 }

                                 if ( $p_query->num_rows > 0 ) {

                                     $p_data = $p_query->fetch_assoc();
                                     $total_view_label = empty($p_data["requested_views"]) ? $p_data["page_view"] : $p_data["requested_views"];
                                     // print_r($p_data) ; echo "<hr/>" ;
                                     
                                     // get page views used
                                     $svc_query = $conn->query(" SELECT id , view_count FROM `site_visit_count` WHERE website_id = '".$sb_pd["id"]."' ORDER BY id ASC LIMIT 1 ; ") ;
                              ...
                              if ( $total_view_label != "Unlimited" ) {
                                 $int_total_view_label = (int)str_replace(',', '', $total_view_label);
                                 $int_view_count = (int)str_replace(',', '', $view_count);

                                 if ( $int_view_count >= $int_total_view_label ) {
                                    $view_count = $total_view_label ;
                                 }
                              }
                              
                              echo $view_count."/".$total_view_label ;
                              
                              ?>
                              <!-- end page views update -->
                           </span>
                        </li>
                        <li class="list-group-item">Consumed URL's  <span class="float-right"><?=$additional_url_num +$url_or;?>/<?=$additional_url_have +3 ;?></span></li>
                        ...
                        <li class="list-group-item">
                           ...
                           <?php } else{  
                              ...
                              if($newappsumo == 0 || empty($newappsumo)){
                              ?>
   ...
   <div class="add-project-data" style="display:none">
      <div class="popup_wrapper">
         <div class="close-popup-btn" style="display:none; cursor: pointer;"><i class="fa fa-times" aria-hidden="true"></i></div>
         <div class="web_list">
            <?php
   ...
   <script>
      $(document).ready(function(){
      	...
      	<?php  if(($user_type == "AppSumo" || $user_type == "Dealify"  || $user_type == "DealFuel") && ($getting_speed == 1)){  ?>
      		Swal.fire({
      		       html:
      			    ...
      			    '<div class="codes"><div class="code__select"><input type="hidden" id="site_id" value="<?=$Sumocode_site?>">'+"<div class='site'><label>Select website</label><?=$websites?></div></div>"+
      			    '<div class=""><input type="text" code_type="code1" class="form-control mb-2 code1" id="sumo1" placeholder="Code 1"/> <button class="btn btn-primary code11 codeVerifyd">Verify</button></div>' +
