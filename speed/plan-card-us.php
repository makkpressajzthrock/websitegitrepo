      ...
      <div class="top__area" >
         ...
         <div class="price-tag">
            <div class="discount__price" >
               <span class="symbol"><?php if($i!=0){?>$<?php }?></span><span class="amount amount<?php echo $data['id'];?>"  subs="<?php echo $data['id'];?>"><?php echo $data['other_country_price']/12;  ?></span> 
               <span class="after month">USD<span class="month-slash">/<?php if($data['id']=='29'){?> </span>Forever</span> <?php }else{?> </span>Month</span>  <?php }?>
               <span class="after year">USD<span class="month-slash">/<?php if($data['id']=='30'){?> </span>Forever</span> <?php }else{?> </span>Month</span>  <?php }?>
            ...
            <div class="original__price">
               <?php if(!empty($data['us_main_p']) && $data['main_p'] != $data['s_price'] ){   ?>
...
To go to page views in google analytics : Go to - Lifecycle > Engagement > Pages & Screens > Pageviews</span></p>
               ...
               <form method="POST" class="subscription_trial_form" action="/payment/init.php">
                  ...
                  <?php 
                   ...
                   //  print_r($list_of_price);
                     if($data['s_type']=='Gold' && $data['interval']=='year') { 
                        ?>
                           ...
                           <select name="selectPricing" id="selectPricing" class="selectPricing">
                              ...
                              <?php
                                 foreach($list_of_price as $key => $row) {
                                  ...
                                  //  print_r($key);
                                    ?>
                                    <option value="<?=$key?>" <?php  if($key=='250000'){ echo 'selected';} ?> 
                                    data-planId="<?=$data['id']?>" > 
                                    <?php 
                                    if($key !='Unlimited' && $key=='250000' ){ echo ($key/1000)."k page views";}
                                    elseif($key !='Unlimited' && $key=='500000'){ echo ($key/1000)."k page views";}   
                                    elseif($key !='Unlimited' && $key!='1000000' ){echo ($key/1000000)." Million page views";}
                                    elseif($key !='Unlimited' && $key!='5000000'){echo ($key/1000000)." Million page views";}
                                    elseif($key=='Unlimited'){ echo "I want unlimited views"; }
                                    ?>
                                    </option>
      ...
      <div class="top-sec-card  freebtn-active <?php if($activate_free == 1 ) { echo 'freebtn-active'; } ?> ">
         ...
         <h2 class="plan-name">
            <span class="year_free"><?php if($plan_frequency_interval == 'year'  && ($pid=='30' || $pid='29')  ){echo "";}elseif($data['interval']=='month'){}else{echo "<p>MOST POPULAR 50% DISCOUNT</p> ";}?></p> </span> 
            <?php if($data['name']=='Free'){
                  echo 'Basic Plan';
               }else{
                  echo $data['name'];
