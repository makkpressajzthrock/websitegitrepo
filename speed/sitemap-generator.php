...
class SitemapGenerator
{
	...
	private function generateFile($pages)
	{
		...
		// check link already exist
		$temp_pages = [] ;

		foreach ($pages as $page) {

			if ( substr_count($page ,"/websitespeedy.com/") >= 2 ) {
				$page = substr_replace($page,"",26,19) ;
			}

			if($page =="https://websitespeedy.com/blogs/google's-pagespeed-insights-tool-apply-recommendations.php")
			{
				$page = "https://websitespeedy.com/blogs/google-pagespeed-insights-tool-apply-recommendations.php";
			}

			if($page =="https://websitespeedy.com/javascript:void(0);")
			{
				continue;
			}
			...
			if ( empty(count($temp_pages)) || (! in_array($page, $temp_pages)) ) {
				$temp_pages[] = $page ;
